...
object ConfigurationUtil {
    private val LOG = logger<ConfigurationUtil>()

    const val UNDEFINED_VERSION = "N/A"
    val STANDARD_AGLIO = if (SystemInfo.isWindows) "aglio.exe" else "aglio"
    ...
    fun guessToolchainVersion(path: String): String {
        ...
        val exePath = "$path/$STANDARD_AGLIO"
        ...
        try {
            ...
            handler.addProcessListener(object : CapturingProcessAdapter() {
                override fun processTerminated(event: ProcessEvent) {
                    if (event.exitCode != 0) {
                        LOG.warn("Couldn't get aglio toolchain version: " + output.stderr)
                    } else {
            ...
            LOG.warn("Can't execute command for getting aglio toolchain version", e)
